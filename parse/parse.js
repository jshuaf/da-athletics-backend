...
module.exports.addTeamFromData = url =>
	co(function* () {
		...
		if (teamData.level === 'Overview') {
			...
			teamData.url = $('.h-menu-active').next().find('a').first().attr('href');
		}
		let programName;
		let programURL;
		let term;
...
module.exports.refreshEvents = ($) => {
	...
	return Promise.map(
		...
		(event) => {
			...
			const rawData = $(event).find('td').toArray().map(
...
module.exports.teamRoster = ($) => {
	...
	rows.slice(1, -1).each((i, athleteRow) => {
		const rowData = $(athleteRow).find('td').map((_, el) => $(el).text()).toArray();
		const athleteData = {};
